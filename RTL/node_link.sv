...
module node_link (
    ...
    //upstream connections (from the local node to the router)
    input flit_t data_i,
    input is_valid_i,
    output logic [VC_NUM-1:0] credits_o,
    output logic [VC_NUM-1:0] is_allocatable_o,
    //downstream connections (from the router to the local node)
    output flit_t data_o,
    output logic is_valid_o,
    input [VC_NUM-1:0] credits_i,
